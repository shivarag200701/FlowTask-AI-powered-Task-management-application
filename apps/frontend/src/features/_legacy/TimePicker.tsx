...
const TimePicker = ({
  ...
  const [timeZoneMode, setTimeZoneMode] = useState<"local" | "floating">(
    "local"
  );
      ...
      >
        <div className="flex flex-col gap-2 p-3">
          <div className="grid grid-cols-[1fr_2fr] gap-2">
            ...
            <button
              ...
              className="w-full p-1 border border-border flex items-center  text-center gap-2 rounded-sm cursor-pointer 
            hover:border-border-hover transition-colors duration-300"
              onClick={() => setIsTimeOptionOpen(!isTimeOptionOpen)}
            >
        ...
        <div className="flex justify-end">
          <div className=" flex justify-between p-3.5 gap-2">
            <button
              className="w-full py-1.5 px-4  bg-muted hover:bg-muted flex items-center  text-center gap-2 rounded-sm cursor-pointer transition-colors duration-300"
              onClick={() => {
                onClose();
              }}
            >
              <div className="text-foreground text-xs font-light text-left">
                Cancel
              </div>
            </button>
            {!isTimeSelected ? (
              <button
                className="w-full py-1.5 px-4 bg-red-600 hover:bg-red-500 flex items-center  text-center gap-2 rounded-sm cursor-pointer
...
const TimeOption = ({ onTimeSelect, onClose, buttonRef }: TimeOptionProps) => {
  ...
  return (
    <>
      ...
      <div
        className="fixed bg-card border border-border rounded-md shadow-2xl z-90 max-h-[200px] overflow-y-auto p-1"
        style={{
          left: `${position.left}px`,
          top: `${position.top}px`,
          marginTop: "4px",
      ...
      >
        {timeOptions.map((time) => (
          <button
            key={time.value}
            className="w-full p-2.5  flex text-left gap-2 rounded-sm cursor-pointer hover:border-border transition-colors duration-300 text-foreground text-sm font-light hover:bg-muted"
            onClick={(e) => {
...
const TimeZonePicker = ({
  ...
  return (
    <>
      <div className="fixed inset-0 z-80" onClick={onClose} />
      <div
        className="fixed bg-card border border-border rounded-md shadow-2xl z-90 max-h-[200px] overflow-y-hidden p-1"
        style={{
          left: `${position.left}px`,
          top: `${position.top}px`,
          marginTop: "4px",
