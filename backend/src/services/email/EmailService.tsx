  ...
  const from = process.env.NODE_ENV === "development" ? "Acme <onboarding@resend.dev>" : process.env.EMAIL
    try{
        ...
        switch (props.template) {
            ...
            case "notification":
            html = await pretty(await render(<NotificationEmail title={props.title} todoId={props.todoId} />))
        ...
        console.log("hrml",props.email);
        

  const { data } = await resend.emails.send({ from, to: props.email, subject, html })
  return data
