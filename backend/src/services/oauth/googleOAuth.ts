...
if (!GOOGLE_CLIENT_ID || GOOGLE_CLIENT_ID.trim() === "") {
  throw new Error(
    "GOOGLE_CLIENT_ID is required but not set in environment variables"
  );
}
if (!GOOGLE_CLIENT_SECRET || GOOGLE_CLIENT_SECRET.trim() === "") {
  throw new Error(
    "GOOGLE_CLIENT_SECRET is required but not set in environment variables"
  );
}
if (!GOOGLE_REDIRECT_URI || GOOGLE_REDIRECT_URI.trim() === "") {
  throw new Error(
    "GOOGLE_REDIRECT_URI is required but not set in environment variables"
  );
}
...
export async function getGoogleTokens(code: string): Promise<{
  ...
  try {
    ...
    if (!userInfo.email || userInfo.email.trim() === "") {
      throw new Error(
        "Google userinfo API did not return a valid email address"
      );
    }
    ...
    throw new Error(
      `Failed to exchange Google OAuth code for tokens: ${String(error)}`
    );
  }
}
