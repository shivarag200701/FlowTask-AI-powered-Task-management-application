...
const CustomDatePicker = ({
  ...
  const recurringOccurenceDates = useMemo(() => {
    ...
    while (iteration < maxIterations) {
      ...
      currentOccurence = calculateNextOccurence(
        todo.recurrencePattern,
        todo.recurrenceInterval,
        currentOccurence,
      );
  ...
  return createPortal(
    <>
      ...
      >
        ...
        <div className="p-2 border-b border-border space-y-1">
          {quickOptions.map((option) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`w-4 h-4 ${isSelected || isNoDate ? "text-white" : "text-muted-foreground"}`}
                />
                <div className="flex-1 text-left">
                  <div
                    className={`text-sm ${isSelected || isNoDate ? "text-white" : "text-muted-foreground"}`}
                  >
                    {option.label}
                  </div>
                </div>
        ...
        {/* Calendar */}
        <div
          className={`overflow-y-auto no-scrollbar  ${parsedResult?.confidence === "high" ? "max-h-[160px]" : "max-h-[250px]"}`}
        >
          {/* Month Header */}
            ...
            {!hoverDate ? (
              ...
              <div className="flex items-center justify-center gap-1 py-1">
                ...
                <div className="text-center text-[10px] text-muted-foreground font-medium ">
                  {getTaskCountForDate(
                    hoverDate?.toLocaleDateString("en-CA").split("T")[0],
                  )}
                </div>
                <div className="text-center text-[10px] text-muted-foreground font-medium">
                  {getTaskCountForDate(
                    hoverDate?.toISOString().split("T")[0],
                  ) == 0
                    ? "tasks"
                    : getTaskCountForDate(
                          hoverDate?.toISOString().split("T")[0],
          ...
          <div className="grid grid-cols-7 gap-1 px-2">
            {days.map((date, idx) => {
              ...
              if (
                (today >= 18 && date.getDate() >= 18) ||
                currentMonth.getMonth() != getTodayMonth()
              ) {
                return (
                  <button
                    onMouseEnter={() => {
                  ...
                  >
                    <div className="flex flex-col items-center">
                      {date.getDate()}
                      {getTaskCountForDate(dateToInput(date)) > 0 && (
          ...
          <div className="mt-6 px-2">
            <div className="text-[13px] text-foreground font-bold mb-2 px-2">
              {nextMonthLabel}
            </div>
        ...
        <div className="flex-col items-center justify-between">
          <div className="border-t border-border p-2.5 text-xs text-muted-foreground flex items-center justify-center text-center gap-2">
            <button
              ref={timeButtonRef}
              className="w-full p-1.5 border border-border flex items-center justify-center text-center gap-2 rounded-sm cursor-pointer group [&:not(:has(.clear-icon:hover))]:hover:bg-secondary transition-colors duration-300 relative"
              onClick={() => {
            ...
            >
              ...
              {!isAllDay ? (
                <div className="text-foreground font-medium">
                  {convert24hrTo12hr(selectedTime)}
                </div>
              ) : (
                <div>Time</div>
              )}
              {!isAllDay && (
                ...
                >
                  <X
                    className="absolute clear-icon cursor-pointer right-0.5 top-1/2 -translate-y-1/2 hover:bg-secondary p-1 h-5.5 w-5.5 rounded-sm "
                    size={13}
                  />
                </button>
              )}
          ...
          <div className="w-full p-2.5 text-muted-foreground text-xs flex items-center justify-center text-center gap-2">
            <button
              ref={reccurenceButtonRef}
              className="w-full p-1.5 border border-border flex items-center justify-center text-center gap-2 rounded-sm cursor-pointer group [&:not(:has(.recurring-clear-icon:hover))]:hover:bg-secondary  transition-colors duration-300 relative"
              onClick={() => {
                setShowReccurencePicker(true);
              }}
            >
              <div className="flex items-center justify-center gap-2">
                ...
                {isRecurring && recurrencePattern && (
                  <span className="font-bold text-foreground">
                    {recurrencePatternMap[recurrencePattern]}
                  </span>
                )}
              </div>
              ...
              {isRecurring && (
                ...
                >
                  <X
                    className="recurring-clear-icon absolute cursor-pointer right-0.5 top-1/2 -translate-y-1/2 hover:bg-secondary p-1 h-5.5 w-5.5 rounded-sm "
                    size={13}
                  />
                </button>
              )}
