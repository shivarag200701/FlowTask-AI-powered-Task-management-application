...
const Landing = () => {
  ...
  return (
    <div
      className="relative  h-full overflow-y-auto bg-white text-white flex flex-col"
      ref={containerRef}
    >
      <style>
      ...
      {/* Header */}
      <header
        className={`transition-all duration-300  fixed bg-white z-50 top-0  border-b border-gray-200 shadow-sm  ${
          isMobile
            ? isScrolled
              ? "bg-white backdrop-blur-md border-b border-white/5 fixed top-0 left-0 right-0 w-full z-50 "
              : "bg-transparent fixed top-0 left-0 right-0 w-full z-50 "
            : ""
        }`}
      ...
      >
        <div className="w-full max-w-6xl mx-auto px-4 py-3">
          <div className="flex items-center sm:justify-between justify-center">
            <div
              className="flex items-center gap-3 transition-all duration-300 ease-in-out cursor-pointer"
              onClick={() => {
            ...
            {!isMobile && (
              <div className="group relative flex  text-black p-1 rounded-lg w-50">
                <a
                  ...
                  className="peer/one flex-1 text-center py-2 z-10 cursor-pointer select-none"
                >
                  Live Demo
                </a>
                <a
                  onClick={scrollToFeatures}
                  className="peer/two flex-1 text-center py-2 z-10 cursor-pointer select-none"
                >
                  Features
                </a>
                <div className="absolute inset-y-1 left-1 w-[calc(50%-4px)] bg-gray-100 rounded-xl transition-all duration-300 ease-in-out peer-hover/two:translate-x-full peer-hover/one:translate-x-0 opacity-0  group-hover:opacity-100"></div>
              </div>
            )}
            <div
      ...
      <main className="relative bg-[#f9fafc] flex-1 flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8 py-20 lg:py-30 2xl:py-64">
        <div className="relative max-w-7xl w-full mx-auto grid gap-12 lg:grid-cols-[0.8fr_1.1fr] items-center z-10">
          <div className="flex flex-col lg:inline-block items-center">
            {/* Main Heading */}
            <h2
              className={`text-5xl sm:text-6xl lg:text-5xl xl:text-6xl 2xl:text-7xl transition-all duration-300 font-bold leading-tight mb-6 text-gray-900 text-center lg:text-left
            ...
            >
              ...
              <br />
              <AuroraText
                className={`${isVisibleTwo ? "translate-y-0 opacity-100" : " translate-y-3 opacity-0 "} transition-all duration-500`}
              >
                Scale Together
              </AuroraText>
            </h2>
            {/* Subheading */}
            <p
              className={`text-xl flex text-gray-500 mb-10 max-w-xl text-center lg:text-left ${isVisibleThree ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"} transition-all duration-700`}
            >
            ...
            {/* CTA Buttons */}
            <div
              className={`flex flex-col sm:flex-row gap-4 mb-10 ${isVisibleFour ? "translate-y-0 opacity-100" : "translate-y-5 opacity-0"} transition-all duration-1000`}
            >
              <button
                onClick={() => navigate("/signup")}
            ...
            <div
              className={`flex gap-5 text-xs lg-text-xs xl:text-sm text-gray-500 ${isVisibleFive ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"} transition-all duration-1500`}
            >
          ...
          <div
            className={`relative ${isVisibleSix ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"} transition-all duration-1300 max-w-6xl px-4 sm:px-6`}
          >
            <img
              src="hero.svg"
              alt="hero.svg"
              className="shadow-2xl bg-[#f9fafc] rounded-xl w-full "
            />
          </div>
        </div>
        ...
        <div
          id="demo"
          className="flex flex-col items-center relative w-full max-w-7xl mx-auto mt-50 lg:mt-50 2xl:mt-90 z-10"
        >
          <section className="w-full relative">
            <div className="absolute -top-1/6 flex">
              ...
              >
                <p className="text-xl sm:text-2xl lg:text-3xl text-center text-slate-600 leading-tight">
                  Take a peek!
                </p>
                <Highlighter
                  action="underline"
                  ...
                  iterations={2}
                >
                  <p className="text-xl sm:text-2xl lg:text-3xl text-slate-600 leading-tight">
                    See Flowtask in action!
                  </p>
                </Highlighter>
              </div>
        ...
        <section
          id="features"
          className="relative w-full max-w-[900px] lg:max-w-[1000px] xl:max-w-[1200px] mx-auto p-8 mt-30 lg:mt-50 2xl:mt-60 z-10"
        >
          ...
          <div className="hidden lg:grid grid-cols-7 grid-rows-4 h-[650px] xl:h-[800px]  text-black gap-4 xl:gap-6 items-stretch">
            ...
            <div className="lg:col-span-3 lg:row-span-2 w-full py-12 lg:p-10 bg-red-50/50 shadow-md border border-gray-200 rounded-2xl flex flex-col p-4 relative overflow-hidden">
              <div className="flex flex-col">
                <h3 className="text-[14px] mb-1 text-gray-800 text-left font-medium">
                  Never miss a beat
                </h3>
                <p className="text-[16px] text-accent font-medium mb-4">
...
export function ArcadeEmbed() {
  return (
    <div
      ...
      style={{
        position: "relative",
        paddingBottom: "calc(55.73099415204679% + 41px)",
        height: "0",
        width: "100%",
        minHeight: "500px",
      }}
