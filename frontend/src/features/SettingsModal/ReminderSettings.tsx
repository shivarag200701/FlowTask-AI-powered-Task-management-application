...
const ReminderSettings = ({
  ...
  }, [preferences, emailEnabled, inAppEnabled, pushEnabled, smsEnabled]);
    ...
    <main className="min-h-[calc(100vh-100px)] max-h-[calc(100vh-100px)] rounded-r-md font-medium overflow-y-auto bg-background relative">
      ...
      <div className="p-6 flex flex-col gap-2">
        ...
        <p className="text-sm text-muted-foreground mb-6">
          Choose your preferred channels for receiving task reminders, daily
          digests, and important updates. We're currently working on expanding
          these options.
        </p>
        {methods.map((method) => (
          <div>
            ...
            <div
              className={`flex justify-between gap-4 ${method.comingSoon ? "opacity-40" : ""}`}
            >
              <div
                className={`flex gap-4 ${method.comingSoon ? "pointer-events-none" : ""}`}
              >
                ...
                <div className="flex flex-col">
                  <div className="flex gap-3">
                    <p>{method.title}</p>
                    {method.comingSoon && (
                      <div className="rounded-lg p-1 border border-border text-[10px] bg-border tracking-wide uppercase">
                        Coming Soon
                      </div>
                    )}
                  </div>
              ...
              <div
                className={`flex items-center ${method.comingSoon ? "cursor-not-allowed" : ""}`}
              >
                <Switch
                  disabled={method.comingSoon}
                  checked={valueMap[method.id]}
                  onClick={() => setterMap[method.id](!valueMap[method.id])}
                />
              </div>
            </div>
          </div>
