...
export const SettingsModal = ({ setShowSettingsModal }: SettingModalProps) => {
  ...
  return (
    <>
      ...
      />
      <div
        className="fixed left-1/2 top-1/2 z-50 bg-sidebar text-foreground rounded-2xl shadow-md border border-border max-w-5xl w-full h-[90vh] max-h-[90vh] overflow-hidden"
        style={{
      ...
      >
        <div className="grid grid-cols-[230px_auto] items-stretch h-full relative">
              ...
              {tabs.map((tab) => (
                <button
                  ...
                  className={`w-full text-[14px] p-1.5 rounded-[4px] cursor-pointer
                            ${
                              tab.id === currentTab
                                ? "bg-accent/15 border-l-2 border-accent text-accent hover:disabled transition-all duration:300"
                                : "hover:bg-options-hover transition-all duration:300 "
                            } `}
                  onClick={() => {
                    if (hasChanged) {
          ...
          {currentTab === "account" && !currentSubTab && (
            <AccountSettings
          ...
          {currentTab === "account" &&
            currentSubTab === "password" &&
            !user.isPasswordSet && (
              <AddPassword handleBack={handleSubTabChange} />
            )}
          {currentTab === "account" &&
            currentSubTab === "password" &&
            user.isPasswordSet && (
              <ChangePassword handleBack={handleSubTabChange} />
            )}
...
export const UnsavedModal = ({
  ...
  return (
    <>
      <div>
        <div
          className="z-50 inset-0 fixed bg-black/30"
          onClick={() => setModalOpen(false)}
        />
        <div
          className="z-60 w-[375px] sm:w-[450px] h-content rounded-md p-4 fixed left-1/2 top-1/4  bg-task"
          style={{
            transform: "translate(-50%, -50%)",
          }}
        >
          <div className="flex flex-col gap-2">
            ...
            <span className="text-foreground font-light mb-6">
              Your unsaved changes will be discarded.
            </span>
            <div className="flex justify-end gap-2">
              <button
                className="px-3 py-1.5 bg-button-subtle text-button-text rounded-sm max-w-30 text-[13px] font-medium hover:bg-button-subtle-hover cursor-pointer"
                onClick={() => {
                  setModalOpen(false);
                }}
              >
