...
const SideBar = ({
  ...
  return (
    ...
    >
      <nav
        className={`h-full flex flex-col bg-sidebar shadow-sm
      ...
      >
        <div className="flex flex-col py-4 px-3 gap-2 min-w-0">
          ...
          {!expanded && (
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  className="p-2 rounded-md hover:bg-hover transition-colors hover:bg-secondary cursor-pointer absolute top-3 left-3 z-50 pointer-events-auto"
                  onClick={() => setExpanded(!expanded)}
                  aria-label="Open sidebar"
                >
                  <PanelLeft
                    className="text-muted-foreground font-light transition-all duration-300 ease-in-out w-4 h-4"
        ...
        {expanded && (
          <div className="px-3 pb-4 border-t border-border pt-3">
            ...
            >
              <Settings
                className="text-muted-foreground font-light w-5 h-5"
                strokeWidth={1.5}
              />
      ...
      {showDropDown && (
        <DropDown
          ...
          setOptionsActive={setMoreOptionsActive}
          width="w-64"
        >
...
export const SideBarItem = ({
  ...
  return (
    <>
      {expanded && (
        <li
          className={`relative flex gap-2 p-2 rounded-sm transition-all duration-300 ease-in-out  cursor-pointer my-1 whitespace-nowrap group
        ...
        >
          ...
          <span
            className={`transition-all duration-300 ease-in-out overflow-hidden w-32 opacity-100`}
          >
            {text}
          </span>
          {expanded && (
